...
// InvalidateList removes all well-known cache keys (call on create/update/delete).
// Menggunakan key eksplisit, bukan wildcard (DEL tidak support pattern).
func (c *SMECache) InvalidateList(ctx context.Context) error {
	// Hapus key umum yang paling sering di-cache
	keys := []string{
		keyAllSMEs,
		"sme:list:cat=:status=:q=",
		"sme:list:cat=food:status=:q=",
		"sme:list:cat=:status=active:q=",
		"sme:list:cat=food:status=active:q=",
	}
...
// GetCategories retrieves the cached category list. Returns (nil, nil) on cache miss.
...
// BuildFilterKey membuat cache key dari parameter filter.
